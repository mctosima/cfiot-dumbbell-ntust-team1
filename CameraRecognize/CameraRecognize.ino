...
void setup() {
  ...
  */
  ...
  int faceCount = read_face_id_from_flash_with_name(&st_face_list);// Read current face data from on-board flash
  Serial.printf("Detected face data: %d face(s)", faceCount);

  Serial.println("Camera ready for recognizing....");
}
...
bool run_face_recognition() {
  ...
  if (net_boxes) {
    if (align_face(net_boxes, image_matrix, aligned_face) == ESP_OK) {
      ...
      /*
      int matched_id = recognize_face(&st_face_list, aligned_face);
      if (matched_id >= 0) {
